...
import (
	...
	"github.com/oruelle/gofpdf/v3/internal/files"
)
...
func TestFpdfImplementPdf(t *testing.T) {
	...
	var _ gofpdf.Pdf = nil //(*gofpdf.Fpdf)(nil)
	var _ gofpdf.Pdf = nil //(*gofpdf.Tpl)(nil)
}

// TestPagedTemplate ensures new paged templates work
func TestPagedTemplate(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("TestPagedTemplate new pdf error: %v", err)
	}
	tpl, err := pdf.CreateTemplate(func(t *gofpdf.Tpl) {
...
// Isssue fixed by initializing internal font in New() function
func TestIssue0116(t *testing.T) {
	...
	pdf, err = gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatal("TestIssue0116", err)
	}
	err = pdf.AddPage()
	if err != nil {
		t.Fatal("TestIssue0116", err)
	}
	err = pdf.SetFont("Arial", "B", 16)
	if err != nil {
		t.Fatal("TestIssue0116", err)
	}
	pdf.Cell(40, 10, "OK")
	...
	err = pdf.Cell(40, 10, "Not OK") // Font not set
	if err != nil {
		t.Fatal("TestIssue0116", err)
	}
}
...
func TestIssue0193(t *testing.T) {
	...
	if err == nil {
		...
		pdf, err = gofpdf.New("P", "mm", "A4", "")
		if err != nil {
			t.Fatal("TestIssue0193", err)
		}
		pdf.AddPage()
		_, err = pdf.RegisterImageOptionsReader("sweden", gofpdf.ImageOptions{ImageType: "png", ReadDpi: true}, rdr)
	}
	if err != nil {
...
// TestExampleFpdf_AddPage demonsrates the generation of headers, footers and page breaks.
func TestExampleFpdf_AddPage(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatal(err)
	}
	...
	for j := 1; j <= 40; j++ {
		err = pdf.CellFormat(0, 10, fmt.Sprintf("Printing line number %d", j),
			"", 1, "", false, 0, "")
		if err != nil {
			t.Fatal(err)
		}
...
// page-breaking.
func TestExampleFpdf_MultiCell(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_SetLeftMargin(t *testing.T) {
	var y0 float64
	var crrntCol int
...
func TestExampleFpdf_SplitLines_tables(t *testing.T) {
	const (
		colCount = 3
		colWd    = 60.0
		marginH  = 15.0
...
// HTML.
func TestExampleFpdf_HTMLBasicNew(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_AddFont(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", example.FontDir())
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_WriteAligned(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", example.FontDir())
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_Image(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_ImageOptions(t *testing.T) {
...
func TestExampleFpdf_RegisterImageOptionsReader(t *testing.T) {
	var (
	...
	opt.ImageType = "png"
	opt.AllowNegativePosition = true
	_, err = pdf.RegisterImageOptionsReader("logo", opt, fl)
	fl.Close()
...
func TestExampleFpdf_SetAcceptPageBreakFunc(t *testing.T) {
	var y0 float64
	var crrntCol int
	loremStr := lorem()

	pdf, err := gofpdf.New("L", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
	const (
...
func TestExampleFpdf_SetKeywords(t *testing.T) {
...
func TestExampleFpdf_Circle(t *testing.T) {
	const (
		thin  = 0.2
		thick = 3.0
	)
...
func TestExampleFpdf_SetAlpha(t *testing.T) {
	const (
		gapX  = 10.0
		gapY  = 9.0
		rectW = 40.0
	...
	for col := 0; col < 4; col++ {
		...
		for row := 0; row < 4; row++ {
			...
			err = pdf.SetAlpha(0.5, modeList[j])
			if err != nil {
				t.Fatalf("Error %v", err)
			}
			pdf.Image(example.ImageFile("golang-gopher.png"),
...
func TestExampleFpdf_LinearGradient(t *testing.T) {
	pdf, err := gofpdf.New("", "", "", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_ClipText(t *testing.T) {
	pdf, err := gofpdf.New("", "", "", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_PageSize(t *testing.T) {
	pdf, err := gofpdf.NewCustom(&gofpdf.InitType{
		UnitStr:    "in",
...
func TestExampleFpdf_Bookmark(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_TransformBegin(t *testing.T) {
	const (
		light = 200
		dark  = 0
	)
...
func TestExampleFpdf_RegisterImage(t *testing.T) {
	const (
		margin = 10
		wd     = 210
		ht     = 297
	...
	for j, str := range fileList {
		...
		infoPtr, err = pdf.RegisterImage(imageFileStr, "")
		if err != nil {
			t.Fatalf("Error %v", err)
		}
...
func TestExampleFpdf_SplitLines(t *testing.T) {
	const (
		fontPtSize = 18.0
		wd         = 100.0
	)
	pdf, err := gofpdf.New("P", "mm", "A4", "") // A4 210.0 x 297.0
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_SVGBasicWrite(t *testing.T) {
	const (
		fontPtSize = 16.0
		wd         = 100.0
...
// alignment.
func TestExampleFpdf_CellFormat_align(t *testing.T) {
	type recType struct {
		align, txt string
	}
	recList := []recType{
	...
	var formatRect = func(pdf *gofpdf.Fpdf, recList []recType) {
		...
		for pageJ := 0; pageJ < 2; pageJ++ {
			...
			linkStr = "https://github.com/oruelle/gofpdf"
		}
	}
	pdf, err := gofpdf.New("P", "mm", "A4", "") // A4 210.0 x 297.0
	if err != nil {
...
func TestExampleFpdf_CellFormat_codepageescape(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "") // A4 210.0 x 297.0
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
// 8-bit font encoding.
func TestExampleFpdf_CellFormat_codepage(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", example.FontDir()) // A4 210.0 x 297.0
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_SetProtection(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
// function.
func TestExampleFpdf_Polygon(t *testing.T) {
	const rowCount = 5
	const colCount = 4
	const ptSize = 36
...
// interactively.
func TestExampleFpdf_AddLayer(t *testing.T) {
...
// server.
func TestExampleFpdf_RegisterImageReader(t *testing.T) {
	...
	const (
		...
		ht       = 297
		fontSize = 15
		urlStr   = "https://github.com/oruelle/gofpdf/blob/master/image/gofpdf.png?raw=true"
	...
	if err == nil {
		...
		infoPtr, err := pdf.RegisterImageReader(urlStr, tp, rsp.Body)
		if err != nil {
			t.Fatalf("Error %v", err)
		}
		if pdf.Ok() {
...
func TestExampleFpdf_Beziergon(t *testing.T) {
...
func TestExampleFpdf_SetFontLoader(t *testing.T) {
	var fr fontResourceType
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
...
func TestExampleFpdf_MoveTo(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_SetLineJoinStyle(t *testing.T) {
	const offset = 75.0
	pdf, err := gofpdf.New("L", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
...
func TestExampleFpdf_DrawPath(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_CreateTemplate(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
	...
	// pdf.SetFont("Times", "", 12)
	template, err := pdf.CreateTemplate(func(tpl *gofpdf.Tpl) {
	...
	template2, err := pdf.CreateTemplate(func(tpl *gofpdf.Tpl) {
		tpl.UseTemplate(template)
		subtemplate, err := tpl.CreateTemplate(func(tpl2 *gofpdf.Tpl) {
...
func TestExampleFpdf_AddFontFromBytes(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_ClipRect(t *testing.T) {
...
func TestExampleFpdf_Rect(t *testing.T) {
	marginCell := 2. // margin of top/bottom of cell
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
...
func TestExampleFpdf_SetJavascript(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_AddSpotColor(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
// contents.
func TestExampleFpdf_RegisterAlias(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
// UTF-8 aliases.
func TestExampleFpdf_RegisterAlias_utf8(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleNewGrid(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_SetPageBox(t *testing.T) {
	...
	pdf, err := gofpdf.New("P", "mm", "A4", "") // 210mm x 297mm
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_SubWrite(t *testing.T) {
...
func TestExampleFpdf_SetPage(t *testing.T) {
	rnd := rand.New(rand.NewSource(0)) // Make reproducible documents
	pdf, err := gofpdf.New("L", "cm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
	pdf.SetFont("Times", "", 12)
...
func TestExampleFpdf_SetFillColor(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_TransformRotate(t *testing.T) {
...
// with utf-8 mode
func TestExampleFpdf_AddUTF8Font(t *testing.T) {
	var fileStr string
	var txtStr []byte
...
func TestExampleUTF8CutFont(t *testing.T) {
...
func TestExampleFpdf_RoundedRect(t *testing.T) {
	const (
		wd     = 40.0
		hgap   = 10.0
		radius = 10.0
	...
	pdf, err := gofpdf.New("P", "mm", "A4", "") // 210 x 297mm
	if err != nil {
		t.Fatalf("Error %v", err)
	}
	pdf.AddPage()
...
// underline thickness.
func TestExampleFpdf_SetUnderlineThickness(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "") // 210mm x 297mm
	if err != nil {
		t.Fatalf("Error %v", err)
	}
...
func TestExampleFpdf_Cell_strikeout(t *testing.T) {
...
func TestExampleFpdf_SetTextRenderingMode(t *testing.T) {
...
func TestIssue0316(t *testing.T) {
	pdf, err := gofpdf.New(gofpdf.OrientationPortrait, "mm", "A4", "")
	if err != nil {
		t.Fatalf("Error %v", err)
	}
	pdf.AddPage()
...
// at the top-level.
func TestExampleFpdf_SetAttachments(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		return
	}
...
func TestExampleFpdf_AddAttachmentAnnotation(t *testing.T) {
	pdf, err := gofpdf.New("P", "mm", "A4", "")
	if err != nil {
		return
	}
...
func TestExampleFpdf_SetModificationDate(t *testing.T) {
	// pdfinfo (from http://www.xpdfreader.com) reports the following for this example :
	// ~ pdfinfo -box pdf/Fpdf_PageBox.pdf
	// Producer:       FPDF 1.7
	// CreationDate:   Sat Jan  1 00:00:00 2000
